...
def main(argv: list[str] | None = None):
    ...
    # ONNX session if --onnx supplied
    onnx_session = None
    provider_req = None if args.provider == "auto" else args.provider
    if args.onnx:
        if not args.onnx.exists():
            logger.error("ONNX file not found: %s (run scripts/export_onnx.py)", args.onnx)
            return 1
        # provider auto → translate to factory's expectation (None = auto)
        try:
            onnx_session = create_session(str(args.onnx), requested=provider_req)
            logger.info("ONNX session active: %s", onnx_session.get_providers())
        except Exception as e:
            logger.error("Failed to create ONNX session: %s", e)
            return 1
        # If provider was auto, we still want to keep provider_req for logging; pipeline will use session
    else:
